...
import { getInstituicaoTransacoes, type InstitutionRedemption, type InstitutionTransaction } from '../services/transacoesService';

export function InstituicaoTransacoesPage() {
  const storedUser = getStoredUser();
  const institutionId = storedUser?.id ?? '';
  ...
  async function load() {
    ...
    try {
      const [data, inst] = await Promise.all([
        getInstituicaoTransacoes(),
        institutionId ? getInstituicao(institutionId) : Promise.resolve(null)
      ]);
      setTransactions(data.transactions);
      setRedemptions(data.redemptions);
      if (inst) setInstitutionName(inst.name);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Nao foi possivel carregar as transacoes');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section className="stack">
      <PageHeader
        title={`Transacoes — ${institutionName || 'Sua Instituicao'}`}
        description="Historico de envios de moedas e resgates dos alunos da sua instituicao."
        actions={
      ...
      <div className="table-card">
        {loading ? (
          <div className="empty-state">Carregando transacoes...</div>
        ) : tab === 'transactions' ? (
          filteredTx.length === 0 ? (
